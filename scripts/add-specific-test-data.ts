...
// Status definitions based on requirements
const STATUSES = {
...
async function addSpecificTestData() {
  console.log('🚀 Adding specific test data to meet requirements...');
  ...
  try {
    ...
    // 1. Create BDR leads with various statuses
    await createBdrLeadsWithVariousStatuses(results, existingEmails, existingNames);
    
    // 2. Create "lists out" with subitems that haven't had a sale yet
    await createListsOutWithoutSales(results, existingEmails, existingNames);
    
    // 3. Create sold lists with sublists containing deal updates
    await createSoldListsWithDealUpdates(results, existingEmails, existingNames);
    
    // 4. Create leads demonstrating conversion from "call booked" to various dispositions
    await createConversionExamples(results, existingEmails, existingNames);
    
    console.log('✅ Specific test data addition completed!');
...
async function createBdrLeadsWithVariousStatuses(results: any, existingEmails: Set<string>, existingNames: Set<string>) {
  console.log('📞 Creating BDR leads with various statuses...');
  ...
  for (let i = 0; i < 60; i++) { // 10 leads per status
    const status = statuses[i % statuses.length];
    const bdr = BDR_NAMES[Math.floor(Math.random() * BDR_NAMES.length)];
    const leadData = await createUniqueLead(i, bdr, status, existingEmails, existingNames);
...
async function createListsOutWithoutSales(results: any, existingEmails: Set<string>, existingNames: Set<string>) {
  console.log('📋 Creating lists out with subitems that haven\'t had a sale yet...');
  
  for (let i = 0; i < 20; i++) { // 20 lists out
    const bdr = BDR_NAMES[Math.floor(Math.random() * BDR_NAMES.length)];
    const leadData = await createUniqueLead(100 + i, bdr, STATUSES.LIST_OUT, existingEmails, existingNames);
...
async function createSoldListsWithDealUpdates(results: any, existingEmails: Set<string>, existingNames: Set<string>) {
  console.log('💰 Creating sold lists with sublists containing deal updates...');
  
  for (let i = 0; i < 15; i++) { // 15 sold lists
    const bdr = BDR_NAMES[Math.floor(Math.random() * BDR_NAMES.length)];
    const leadData = await createUniqueLead(200 + i, bdr, STATUSES.SOLD, existingEmails, existingNames);
...
async function createConversionExamples(results: any, existingEmails: Set<string>, existingNames: Set<string>) {
  console.log('🔄 Creating conversion examples from call booked to various dispositions...');
  ...
  for (let i = 0; i < 40; i++) { // 10 examples per disposition
    const disposition = dispositions[i % dispositions.length];
    const bdr = BDR_NAMES[Math.floor(Math.random() * BDR_NAMES.length)];
    const leadData = await createUniqueLead(300 + i, bdr, disposition, existingEmails, existingNames);
...
async function createUniqueLead(index: number, bdr: string, status: string, existingEmails: Set<string>, existingNames: Set<string>) {
  let attempts = 0;
  ...
  while (attempts < maxAttempts) {
    ...
    const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}.test${index}@${company.toLowerCase().replace(/\s+/g, '')}.com`;
    const name = `${firstName} ${lastName} (Test ${index})`;
    ...
    if (!existingEmails.has(email) && !existingNames.has(name.toLowerCase())) {
      ...
      return {
        lead: {
          ...
          status: status,
          link: `https://linkedin.com/in/${firstName.toLowerCase()}-${lastName.toLowerCase()}-test${index}`,
  ...
  console.log(`⚠️ Could not generate unique lead data after ${maxAttempts} attempts for index ${index}`);
  return null;
}
