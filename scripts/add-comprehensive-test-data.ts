...
async function addComprehensiveTestData(config: TestDataConfig) {
  ...
  const futureCalls = await createFutureCalls(endDate, BDR_NAMES, existingEmails, existingNames);
  ...
  try {
    ...
    console.log(`📈 Added: ${mainLeads.length} leads, ${pipelineItems.length} pipeline items, ${activityLogs.length} activities`);
...
function determineSalesProgression(startDate: Date, currentDate: Date) {
  ...
  if (callHappened) {
    stages.push({
      ...
      newStatus: SALES_PROCESS_STATUSES.CALL_COMPLETED
    });
  ...
  if (callHappened) {
    ...
    if (outcome !== SALES_PROCESS_STATUSES.DECLINED) {
      ...
      stages.push({
        ...
        previousStatus: SALES_PROCESS_STATUSES.CALL_COMPLETED,
        newStatus: outcome
      });
...
async function createFutureCalls(endDate: Date, bdrNames: string[], existingEmails: Set<string>, existingNames: Set<string>) {
  ...
  for (let i = 0; i < futureCallCount; i++) {
    ...
    const lastName = generateLastName();
    const company = generateCompanyName();
    const email = `future.contact${i}.test@${company.toLowerCase().replace(/\s+/g, '')}.com`;
    const name = `${firstName} ${lastName} (Future Test ${i})`;
...
const CONFIG: TestDataConfig = {
  totalMainLeads: 200,           // 200 main leads
...
export { addComprehensiveTestData, CONFIG }; 